...
def map_lines_to_vectors(lines: list) -> list:
	...
	return list(
		map(
			lambda line: [get_point_from_string(s) for s in split_line_coords(line)],
			lines,
		)
	)
...
def find_coverage(start: list, end: list, ignore_diagonals: bool = True) -> list[tuple]:
	...
	for idx in (0, 1):
		if start[idx] == end[idx]:
			...
			return [
				(start[idx], i) if idx == 0 else (i, start[idx])
				for i in range(start[idx - 1], end[idx - 1] + 1)
			]
	...
	if not ignore_diagonals:
		...
		return [
			(start[0] + i, start[1] + i * vertical_step) for i in range(0, end[0] - start[0] + 1)
		]
...
if __name__ == '__main__':
	print(f'One star solution in test is {count_double_overlaps(TEST_FILE, False)}')
	print(f'Two star solution in test is {count_double_overlaps(TEST_FILE, True)}')
	print(f'One star solution is {count_double_overlaps(INPUT_FILE, False)}')
	print(f'Two star solution is {count_double_overlaps(INPUT_FILE, True)}')
