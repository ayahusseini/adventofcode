...
def load_data(filename: str) -> tuple[list[int], list[str]]:
    ...
    order = convert_string_to_list_of_ints(
        lines.pop(0).replace("\n", ""), ",")
    ...
    boards = [board_lines[i: i+5]
              for i in range(len(board_lines)) if i % 5 == 0]
...
def find_matching_index(board: list[list], target: int, matches: list = []) -> list[tuple]:
...
def find_winning_score(board_list: list, numbers_to_draw: list[int]) -> int:
    ...
    for num in numbers_to_draw:
        for i, board in enumerate(board_list):
            ...
            board_matches[i] = find_matching_index(
                board, num, board_matches[i])
...
def find_lowest_score(board_list: list, numbers_to_draw: list[int]) -> int:
    ...
    return sum_unmarked_matches(board_list[already_won[-1]], board_matches[already_won[-1]])*nums_won_on[-1]
