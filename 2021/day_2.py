...
def map_direction_to_new_orientation_two_star(direction_name: str, current_aim: int):
    return np.array([1, current_aim]) if direction_name == "forward" else np.array([0, 0])
...
def get_final_position_one_star(instructions: list[list]):
    ...
    return sum([i[1] * map_direction_to_new_orientation_one_star(i[0])
...
def get_final_position_two_stars(instructions: list[list]):
    ...
    for i in instructions:
        current_pos = sum([current_pos, i[1] *
                           map_direction_to_new_orientation_two_star(i[0], current_aim)])
