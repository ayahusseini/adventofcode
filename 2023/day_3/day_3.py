...
class Grid:
    ...
    def get_part_number_for_idx(self, idx: tuple):
        ...
        adjacent = [
            idx
            for idx in self.get_adjacent(idx)
            if self.rows[idx[0]][idx[1]].isnumeric()
        ]
        return [
            self.extract_full_num(i, cache=True)
            for i in adjacent
    ...
    def extract_full_num(self, idx: tuple, cache: bool = False) -> int:
        ...
        behind = self.traverse_row(idx[0], idx[1] - 1, -1, lambda x: x.isnumeric())[
            ::-1
        ]
        ...
        if cache:
            self.marked.update(
                [(idx[0], idx[1] + n) for n in range(-len(behind), len(front) + 1)]
            )
        return number
