...
class Grid:
	...
	def get_part_number_for_idx(self, idx: tuple):
		...
		adjacent = [idx for idx in self.get_adjacent(idx) if self.rows[idx[0]][idx[1]].isnumeric()]
		return [self.extract_full_num(i, cache=True) for i in adjacent if i not in self.marked]
	...
	def get_adjacent(self, idx: tuple):
		...
		return [
			(r, c)
			for r in range(idx[0] - 1, idx[0] + 2)
	...
	def extract_full_num(self, idx: tuple, cache: bool = False) -> int:
		...
		behind = self.traverse_row(idx[0], idx[1] - 1, -1, lambda x: x.isnumeric())[::-1]

		front = self.traverse_row(idx[0], idx[1] + 1, +1, lambda x: x.isnumeric())

		number = int(''.join(behind + [row[idx[1]]] + front))
		if cache:
			self.marked.update([(idx[0], idx[1] + n) for n in range(-len(behind), len(front) + 1)])
		return number
