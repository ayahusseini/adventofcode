...
@pytest.mark.parametrize("line, id", [
...
@pytest.mark.parametrize("line, subres", [
...
def test_get_game_from_line():
    g = Game.from_line(
        "Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green")
    assert g.id == 1
    assert g.subsets == [{"blue": 3, "red": 4, "green": 0},
...
def test_get_game_from_line_three_digit_id():
    g = Game.from_line(
        "Game 111: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green")
    assert g.id == 111
    assert g.subsets == [{"blue": 3, "red": 4, "green": 0},
                         {"red": 1, "green": 2, "blue": 6},
                         {"green": 2, "blue": 0, "red": 0}]
