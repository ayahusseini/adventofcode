...
def get_moving_time(move_time: int, rest_time: int, tot_time: int):
    ...
    return (tot_time // (move_time + rest_time)) * move_time + get_moving_time(move_time, rest_time, tot_time % (move_time + rest_time))
...
def one_star(filename: str, tot_time: int):
    ...
    for l in lines:
        words = l.split(" ")
        distances.append(get_tot_distance(
            int(words[3]), int(words[6]), int(words[-2]), tot_time))
...
def get_scores_after_n_sec(lines: list, scores=[], n: int = 1):
    ...
    for i, l in enumerate(lines):
        words = l.split(" ")
        distances.append(get_tot_distance(
            int(words[3]), int(words[6]), int(words[-2]), n))
