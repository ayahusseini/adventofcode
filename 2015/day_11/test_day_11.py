...
def test_basic_count_subsequences_not_overlapping():
	assert count_subsequences('aabbcc', 2, False, lambda x: x.count(x[0]) == len(x)) == 3


def test_basic_count_subsequences_not_overlapping2():
	assert count_subsequences('aaabbcc', 2, False, lambda x: x.count(x[0]) == len(x)) == 3
...
def test_basic_count_subsequences_overlapping3():
	assert count_subsequences('aaabbcc', 2, True, lambda x: x.count(x[0]) == len(x)) == 4
