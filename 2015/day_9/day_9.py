...
class UndirectedGraph:
	...
	def is_valid_path(self, path: list[str], visit_each_node_once: bool = True):
		...
		valid_edges = all(self.is_valid_edge(path[i], path[i + 1]) for i in range(len(path) - 1))
...
def one_star(filename: str):
	...
	return min(
		get_shortest_path_from_start(graph, node, clear_visited=True) for node in graph.get_nodes()
	)
...
def two_star(filename: str):
	...
	return max(
		get_shortest_path_from_start(graph, node, clear_visited=True, edge_multiplier=-1)
