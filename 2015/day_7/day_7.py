...
def get_in_degrees(lines: list[dict]):
	...
	return {
		i: len(list(filter(lambda x: not isinstance(x, int), l['in']))) for i, l in enumerate(lines)
	}
...
def get_signal_value(lines: list[dict], wire_name: str, override_wire: dict = {}):
	...
	for l in lines:
		input_values = [i if isinstance(i, int) else wire_values.get(i) for i in l['in']]
