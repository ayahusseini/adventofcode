...
def num_ways_to_reach_idx_one_step(
    idx: tuple[int, int],
    is_beam: NDArray[np.int_ | np.bool_],
    is_splitter: np.NDArray[np.int_ | np.bool_],
) -> NDArray[np.int_]:
    """Return the number of ways for a beam to reach idx in one step"""
    is_splitter = is_splitter.astype(bool)
    is_splitter_idcs = np.where(is_splitter)
...
def get_next_to_splitter(is_splitter: NDArray[np.int_ | np.bool_]) -> NDArray[np.int_]:
    ...
    padded = np.pad(is_splitter, pad_width=1, mode="constant", constant_values=0)
